...
#define _MODE MODE_0
#define _PIXEL_FORMAT PIXEL_FORMAT_MONO8 //8 bit raw data output of sensor.
...
void PrintCameraInfo(CameraInfo* pCamInfo)
{
	...
	cout << "Interface Type - " << pCamInfo->interfaceType << endl;
...
int RunGrabLoop(Camera *pCamera)
{
    ...
    while (_kbhit() == 0)
    {
		...
		pixel_info = image.GetBitsPerPixel();
        imageCount++;
    }
	/*
		A method define in Image class: virtual unsigned int GetBitsPerPixel() const
		may help to get the data in each pixel
		and the _BYTES_PER_PIXEL is defined on the top;
	*/
...
int RunCustom()
{
    ...
    if (camInfo.interfaceType != 2) // usb 3.0 interface. before was IEEE-1394 (Includes 1394a and 1394b)
    {
        cout << "Only 1394 cameras are supported in this example." << endl;
		cout << "Is this wired error goes here 1" << endl;
        return -1;
    }
    ...
    fmt7ImageSettings.mode = _MODE; // standard imaging mode with full resolution readout
    fmt7ImageSettings.offsetX = 0;
    fmt7ImageSettings.offsetY = 0;
    fmt7ImageSettings.width = _COLS;
    fmt7ImageSettings.height = _ROWS;
    fmt7ImageSettings.pixelFormat = _PIXEL_FORMAT; //8 bit raw data output of sensor.
...
/* ************************ Run another mode STANDARD if not customise image  *****************************/
int RunStandard()
{
    FC2Config config;
    bool isSupported = false;
