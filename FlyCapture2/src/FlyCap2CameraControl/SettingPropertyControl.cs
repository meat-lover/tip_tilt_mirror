...
namespace FlyCap2CameraControl
{
    internal partial class SinglePropertyControl : UserControl
    {
        ...
        public void SetCameraProperty(float value, bool isAutoModeOn, bool isAbsModeOn)
        {
            try
            {
                CameraProperty camProp = m_camera.GetProperty(m_propType);

                camProp.absControl = isAbsModeOn;
                camProp.autoManualMode = isAutoModeOn;
                ...
                {
                    camProp.valueA = Convert.ToUInt32(value);
                }
            ...
            {
                Debug.WriteLine(string.Format("Unable to set {0} preset status.", m_propType));
