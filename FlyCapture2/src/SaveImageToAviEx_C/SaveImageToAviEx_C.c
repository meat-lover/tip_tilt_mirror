...
#include "C/FlyCapture2Video_C.h"
#include <stdio.h>

typedef enum _VideoType { UNCOMPRESSED, MJPG, H264 } VideoType;
...
int SaveVideoHelper(fc2Context context, VideoType videoType, float frameRate)
{
    fc2Error error;
    ...
    error = fc2VideoCreate(&videoContext);
    if (error != FC2_ERROR_OK)
    {
        printf("Error in fc2VideoCreate: %s\n", fc2ErrorToDescription(error));
        return -1;
    }

    error = fc2VideoSetMaximumSize(videoContext, k_videoFileSize);
    if (error != FC2_ERROR_OK)
    {
        printf("Error in fc2VideoSetMaximumSize: %s\n", fc2ErrorToDescription(error));
        return -1;
    }
    ...
    for (i = 0; i < k_numImagesToGrab; i++)
    {
        ...
        if (i == 0)
        {
            ...
            case UNCOMPRESSED:
                ...
                error = fc2VideoAVIOpen(
                    videoContext, "SaveImageToAviEx_C-Uncompressed", &aviOption);
                if (error != FC2_ERROR_OK)
                {
            ...
            case MJPG:
                ...
                // extensions specified in the filepath
                error = fc2VideoMJPGOpen(
                    videoContext, "SaveImageToAviEx_C-MJPG", &mjpgOption);
                if (error != FC2_ERROR_OK)
                {
                    printf("Error opening video: %s\n", fc2ErrorToDescription(error));
                }
            ...
            case H264:
                ...
                error = fc2VideoH264Open(
                    videoContext, "SaveImageToAviEx_C-H264", &h264Option);
                if (error != FC2_ERROR_OK)
                {
                    printf("Error opening video: %s\n", fc2ErrorToDescription(error));
                }
        ...
        error = fc2VideoAppend(videoContext, &rawImage);
        if (error != FC2_ERROR_OK)
        {
            printf("Error appending to video: %s\n", fc2ErrorToDescription(error));
        }
    ...
    error = fc2VideoDestroy(videoContext);
    if (error != FC2_ERROR_OK)
    {
        printf("Error in fc2VideoDestroy: %s\n", fc2ErrorToDescription(error));
        return -1;
    }
...
int RunCamera(fc2Context context, fc2PGRGuid guid)
{
    ...
    SaveVideoHelper(context, UNCOMPRESSED, frameRate);
    SaveVideoHelper(context, H264, frameRate);
    SaveVideoHelper(context, MJPG, frameRate);
