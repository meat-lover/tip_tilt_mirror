...
UINT RecordingDialog::ThreadSaveFrameHelper()
{
    ...
    {
        FlyCapture2Video fc2Video;

        // Set AVI Split size
        fc2Video.SetMaximumFileSize(m_videoSettings.aviSize);

        switch (m_videoSettings.videoFormat)
        {
        case VideoRecordingPage::UNCOMPRESSED:
            error = fc2Video.Open(m_videoSettings.filename, &(m_videoSettings.aviOption));
            break;
        case VideoRecordingPage::MJPEG:
            error = fc2Video.Open(m_videoSettings.filename, &(m_videoSettings.mjpgOption));
            break;
        case VideoRecordingPage::H264:
            error = fc2Video.Open(m_videoSettings.filename, &(m_videoSettings.h264Option));
            break;
        default:
        ...
        while (GetRecorderState() == STARTED)
        {
            ...
            try
            {
                SaveImageToVideo(&fc2Video, &image);
            }
            catch (RecordingException& e)
            {
                CString errMsg;
...
void RecordingDialog::SaveImageToVideo(FlyCapture2Video* fc2Video, Image* image)
{
    Error error = fc2Video->Append(image);
    if (error != PGRERROR_OK)
    {
        throw RecordingException(error.GetDescription());
    }
